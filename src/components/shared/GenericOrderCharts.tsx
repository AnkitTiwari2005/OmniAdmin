    ...
    <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
      ...
      <div className="glass-card card-highlight rounded-2xl p-6 border border-border/50 hover-lift relative overflow-hidden group">
        <div className="flex items-center justify-between mb-5">
          <div>
            <h3 className="font-bold text-sm text-foreground">Order Volume</h3>
            <p className="text-xs text-muted-foreground mt-0.5">Daily orders placed over the last 7 days</p>
          </div>
          <div
        ...
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            ...
            <Tooltip
              ...
              formatter={(v: number) => [<span className="font-bold text-foreground">{v}</span>, 'Orders']}
            />
            <Bar
              dataKey="orders"
              fill={accentHex}
      ...
      <div className="glass-card card-highlight rounded-2xl p-6 border border-border/50 hover-lift relative overflow-hidden group">
        <div className="flex items-center justify-between mb-5">
          <div>
            <h3 className="font-bold text-sm text-foreground">Revenue Trend</h3>
            <p className="text-xs text-muted-foreground mt-0.5">Gross revenue generated over the last 7 days</p>
          </div>
          <div
            className="flex h-8 w-8 items-center justify-center rounded-xl border border-border/50 shadow-2xs"
            style={{ backgroundColor: accentHex + '18', color: accentHex }}
        ...
        <ResponsiveContainer width="100%" height={220}>
          <AreaChart data={data} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
            <defs>
              <linearGradient id={`grad-${accentHex.replace('#', '')}`} x1="0" y1="0" x2="0" y2="1">
            ...
            <Area
              ...
              strokeWidth={2.5}
              fillOpacity={1}
              fill={`url(#grad-${accentHex.replace('#', '')})`}
              isAnimationActive={true}
              animationDuration={900}
            />
          </AreaChart>
