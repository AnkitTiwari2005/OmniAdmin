...
import { createShudhhamProduct, updateShudhhamProduct, deleteShudhhamProduct } from '@/integrations/shudhham/actions';
...
const EMPTY = { name: '', description: null as string | null, price: 0, category: '', image_url: null as string | null };

export function ShudhhamProductsPanel({ products, total, categories, currentCategory, currentSearch }: Props) {
  ...
  function handleDelete(id: string) {
    ...
    startTransition(async () => {
      await deleteShudhhamProduct(id);
      router.refresh();
    });
  }
      ...
      <div className="rounded-xl border bg-card shadow-sm overflow-hidden">
        <Table>
          ...
          <TableBody>
            {products.length === 0 ? (
              <TableRow>
                <TableCell colSpan={5} className="text-center py-12 text-muted-foreground">No products found</TableCell>
              </TableRow>
            ) : (
              products.map((p) => (
                <TableRow key={p.id}>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <Input value={form.category} onChange={(e) => setForm((f) => ({ ...f, category: e.target.value }))} required placeholder="e.g. Oils" />
              </div>
            </div>
            <div className="space-y-1.5">
